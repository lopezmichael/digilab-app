...
`sync_limitless.py` used `MAX(match_id)+1` for explicit ID inserts, which doesn't advance PostgreSQL's IDENTITY sequence. When OCR-submitted matches relied on the sequence (no explicit ID), they collided on `matches_pkey`. The error handler on line 944 caught ALL `unique|duplicate` errors — including primary key collisions — as duplicate match submissions, silently skipping valid data. Production sequence was 154 behind MAX(match_id).

### Fix: Three Workstreams
1. **Sync script** (`sync_limitless.py`): Removed `MAX(match_id)+1` pattern from both `sync_tournament_data()` and `repair_tournament()`. Let IDENTITY generate IDs. Added safety-net `setval()` after all match inserts.
2. **Submit handler** (`submit-match-server.R`): Replaced plain INSERT with `ON CONFLICT DO UPDATE` (player rows) and `ON CONFLICT DO NOTHING` (mirror rows). Removed broad `grepl("unique|duplicate")` catch-all. Added savepoint around mirror INSERTs.
3. **UI improvements**: Tournament completeness badges (Complete/Partial/No data), existing data preview table, detailed toast messages (new vs updated counts).

### Key Insight
The `GENERATED BY DEFAULT AS IDENTITY` sequence only advances on DEFAULT inserts (no explicit ID). Any script that supplies explicit IDs must call `setval()` afterward, or the sequence will lag and collide with future default inserts.
...
Added `match_type` (normal/bye/default) and `source` (limitless/ocr/manual) columns to matches table. Made `opponent_id` nullable for byes/defaults. Backfilled 38,305 Limitless matches as `source='limitless'` and 19 OCR matches as `source='ocr'`. Also backfilled 19 mirror rows for existing single-perspective OCR matches. Had to fix the `match_id` sequence (`setval` to MAX) before the mirror backfill INSERT — the `GENERATED BY DEFAULT AS IDENTITY` sequence was behind the manually-inserted IDs from Limitless sync.

### Layout-Aware Match History Parser
Ported the bounding-box approach from `parse_standings_layout` to match history. Key difference: match history has 4 columns (Round, Opponent, Results, Points) vs standings' 3 (Ranking, Username, Points). Column boundaries detected dynamically from header keyword positions ("Round", "Opponent", "Results", "Points"). Falls back to hardcoded defaults when header is scrolled off-screen.

Tested against 10 screenshots from iPhone. Layout parser matched or beat the text parser on every screenshot — caught 4th-round matches that text parser missed on 3 screenshots. The "Win by Default" detection needed a fix: GCV places "Win" in the round column X-range, so the opponent column only sees "by Default". Added partial pattern match.

### Mirror Rows for Local Submissions
Match-by-match submit now inserts both perspectives (player + opponent with flipped W/L), matching what `sync_limitless.py` has always done. Mirror INSERT wrapped in its own SAVEPOINT — without this, a failed mirror INSERT (e.g., if opponent already submitted) would poison the transaction and cause subsequent match inserts to fail.

### Risk: Android/Different Phone Screens
The layout parser normalizes all coordinates to percentages, so resolution differences don't matter. The real risk is different aspect ratios or different Bandai TCG+ app layouts on Android. When the header row is visible, column boundaries are dynamic. When scrolled off, hardcoded defaults (calibrated to iPhone) apply. Text parser fallback mitigates total failure. Monitoring via existing verbose `message()` logging in Posit Connect logs.
