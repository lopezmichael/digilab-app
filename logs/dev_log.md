...
**RC1 — `as.numeric(NULL)` crash (171+ events/day):** `input$players_min_events` and `input$meta_min_entries` are pill toggle inputs rendered inside `renderUI`. They start as `NULL` until dynamic UI loads. `as.numeric(NULL)` → `numeric(0)` → `is.na(numeric(0))` → `logical(0)` → `if(logical(0))` → "argument is of length zero". Fixed with `%||% 0` fallback + `length()` check in `public-players-server.R` (lines 130, 370) and `public-meta-server.R` (line 53).

**RC2 — Connection pool prepared statement collisions (~15 events/day):** RPostgres caches prepared statements per connection. When `pool` reuses a connection, stale prepared statements cause "Query requires N params; M supplied" and "invalid input syntax" errors. `safe_query` already retried on "prepared statement" errors but didn't match these messages. Added `grepl("Query requires \\d+ params")` and `grepl("invalid input syntax")` to `is_prepared_stmt_error()` in both `safe_query` and `safe_execute`.

**RC3 — Duplicate key race condition (8 events):** `sync_limitless.py` used `SELECT MAX(request_id) + 1` for manual ID generation — concurrent inserts could get the same ID. Fixed by removing explicit `request_id` from INSERT and using `RETURNING request_id` to let the DB auto-generate via `GENERATED BY DEFAULT AS IDENTITY`.

Also fixed `setNames(NULL, ...)` crash in `public-submit-server.R:1493` match history form.
